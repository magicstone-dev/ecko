...
if Rails.env.development?
  ...
  User.where(email: "admin@#{domain}").first_or_initialize(email: "admin@#{domain}", password: 'eckoadmin123', password_confirmation: 'eckoadmin123', confirmed_at: Time.now.utc, admin: true, account: admin, agreement: true, approved: true).save!
end
